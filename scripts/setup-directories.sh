...
setup_directories() {
    ...
    local vscode_root_link="$HOME/.vscode"
    local vscode_data_root="$vscode_server_root/data"
    ...
    mkdir -p ~/.config/nvim \
        ...
        "$vscode_server_root/extensions" "$vscode_server_root/bin" \
    ...
    if [ ! -L "$vscode_root_link" ]; then
        if [ -d "$vscode_root_link" ]; then
            if [ "$(find "$vscode_root_link" -mindepth 1 -print -quit 2>/dev/null)" ]; then
                echo "ℹ️  Migrating existing VS Code data into ~/.vscode-server"
                cp -a "$vscode_root_link/." "$vscode_server_root/" 2>/dev/null || true
            fi
            rm -rf "$vscode_root_link"
        else
            rm -f "$vscode_root_link"
        fi
        ln -sfn "$vscode_server_root" "$vscode_root_link"
        echo "✅ Linked ~/.vscode to ~/.vscode-server"
    fi

    if [ ! -L "$host_code_config" ]; then
        if [ -d "$host_code_config" ]; then
            if [ "$(find "$host_code_config" -mindepth 1 -print -quit 2>/dev/null)" ]; then
                echo "ℹ️  Migrating existing VS Code config into ~/.vscode-server/data"
        ...
        echo "✅ Linked VS Code config to ~/.vscode-server/data"
    fi
}
